...
impl Node {
    ...
    #[inline]
    pub fn h(&self) -> u64 {
        let value = flatdata_read_bytes!(u64, self.data.as_ptr(), 120, 64);
        unsafe { std::mem::transmute::<u64, u64>(value) }
    }
...
/// The values of the range are indexes in the `tags_index` vector.
    ...
    pub fn tags(&self) -> std::ops::Range<u64> {
        let start = flatdata_read_bytes!(u64, self.data.as_ptr(), 184, 40);
        let end = flatdata_read_bytes!(u64, self.data.as_ptr(), 184 + 28 * 8, 40);
        start..end
    }
...
impl std::cmp::PartialEq for Node {
    #[inline]
    fn eq(&self, other: &Self) -> bool {
        self.id() == other.id() &&        self.lat() == other.lat() &&        self.lon() == other.lon() &&        self.h() == other.h() &&        self.tag_first_idx() == other.tag_first_idx()     }
}
...
impl Node {
    ...
    pub fn set_h(&mut self, value: u64) {
        flatdata_write_bytes!(u64; value, self.data, 120, 64)
    }
